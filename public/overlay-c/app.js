...
(function () {
  ...
  function renderPlayers(players, phase) {
    ...
    players.forEach((p, i) => {
      ...
      row.className = 'mn-p' + (p.alive ? '' : ' dead') + (p.isImpostor ? ' impostor' : '');
      row.style.animationDelay = (i * 0.05) + 's';

      // Dot color
      let dotClass = 'mn-p-dot ';
      if (p.isImpostor) dotClass += 'imp-dot';
      else if (!p.alive) dotClass += 'dead-dot';
      else dotClass += 'alive';

      const wordHtml = p.word && phase !== 'lobby' ? `<span class="mn-p-word">${escapeHtml(p.word)}</span>` : '';
      const impTag = p.isImpostor ? '<span class="mn-p-imp-tag">IMPOSTOR</span>' : '';

      row.innerHTML = `
        <span class="${dotClass}"></span>
        <span class="mn-p-name">${escapeHtml(p.name)}</span>
        ${wordHtml}
        ${impTag}
      `;
      overlayPlayers.appendChild(row);
    });
  }

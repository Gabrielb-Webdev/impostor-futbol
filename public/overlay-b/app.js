...
(function () {
  ...
  function renderPlayers(players, phase) {
    ...
    players.forEach((p, i) => {
      ...
      chip.className = 'bb-chip' + (p.alive ? '' : ' dead') + (p.isImpostor ? ' impostor' : '');
      chip.style.animationDelay = (i * 0.05) + 's';

      let tagHtml = '';
      if (p.isImpostor) {
        tagHtml = '<span class="bb-chip-tag tag-impostor">🕵️</span>';
      } else if (!p.alive) {
        tagHtml = '<span class="bb-chip-tag tag-dead">✕</span>';
      } else {
        tagHtml = '<span class="bb-chip-tag tag-alive">✓</span>';
      }

      const wordHtml = p.word && phase !== 'lobby' ? `<span class="bb-chip-word">${escapeHtml(p.word)}</span>` : '';

      chip.innerHTML = `
        <span class="bb-chip-icon ${p.alive ? 'alive' : 'dead-icon'}">${p.isImpostor ? '🕵️' : (p.alive ? '⚽' : '💀')}</span>
        <div class="bb-chip-info">
          <span class="bb-chip-name">${escapeHtml(p.name)}</span>
          ${wordHtml}
        </div>
        ${tagHtml}
      `;
      overlayPlayers.appendChild(chip);
    });
  }

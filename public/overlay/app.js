...
(function () {
  ...
  socket.on('overlay:state', (state) => {
    ...
    // Ball animation between rounds
    if (lastPhase === 'result' && state.phase === 'role_reveal') {
      triggerBall();
    }
  ...
  function renderPlayers(players, phase) {
    ...
    players.forEach((p, i) => {
      ...
      div.className = 'ov-player' + (p.alive ? '' : ' dead') + (p.isImpostor ? ' impostor' : '');
      div.style.animationDelay = (i * 0.08) + 's';

      let tagHtml = '';
      if (p.isImpostor) {
        tagHtml = '<span class="ov-tag tag-impostor">🕵️ IMPOSTOR</span>';
      } else if (!p.alive) {
        tagHtml = '<span class="ov-tag tag-dead">ELIMINADO</span>';
      } else {
        tagHtml = '<span class="ov-tag tag-alive">VIVO</span>';
      }

      // Mostrar palabra asignada debajo del nombre
      const wordHtml = p.word && phase !== 'lobby' ? `<span class="ov-word">${escapeHtml(p.word)}</span>` : '';

      div.innerHTML = `
        <div class="ov-icon ${p.alive ? 'alive' : 'dead'}">${p.isImpostor ? '🕵️' : (p.alive ? '⚽' : '💀')}</div>
        <div class="ov-info">
          <span class="ov-name">${escapeHtml(p.name)}</span>
          ${wordHtml}
        </div>
        ${tagHtml}
      `;
      overlayPlayers.appendChild(div);
    });
  }
  ...
  function showAlert(data) {
    const verdictClass = data.wasImpostor ? 'is-impostor' : 'is-futbolista';
    ...
    alertContent.innerHTML = `
      ...
      <div class="alert-verdict ${verdictClass}">${verdictText}</div>
  ...
  socket.on('game:over', (data) => {
    const titleClass = data.winner === 'futbolistas' ? 'win-futbolistas' : 'win-impostors';
  ...
  function triggerConfetti(mainColor) {
    confettiPieces = [];
    const colors = [mainColor, '#888888', '#ffffff', mainColor, '#555555'];
    ...
    setTimeout(() => {
      confettiPieces.forEach(p => p.opacity -= 0.02);
    }, 6000);
